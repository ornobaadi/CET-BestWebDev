  ...
  useEffect(() => {
    const fetchAgencies = async () => {
      try {
        ...
        const designAgencies = data.filter((agency: Agency) => agency.category === 'Design');
        
        setAgencies(designAgencies);
        setFilteredAgencies(designAgencies);
      } catch (error) {
  ...
  return (
    <section className="bg-gray-100 py-16">
      <div className="container mx-auto">
        <h2 className="text-3xl font-bold mb-8">Design Agencies</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredAgencies.length > 0 ? (
            ...
            <p className="text-center text-gray-500 col-span-full">No design agencies found.</p>
          )}
        </div>
      </div>
    </section>
